package com.sdu.financesoft.dao;
...
import com.sdu.financesoft.model.*;
...
public class OutaccountDAO {
    ...
    public double getAllFoodMoney() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery(
                "select sum(money) from tb_outaccount where type ='饮食'", null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
        return 0;// 如果没有数据，则返回0
    }

    public double getAllStudyMoney() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery(
                "select sum(money) from tb_outaccount where type ='学习'", null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
    ...
    public double getAllIShoppingMoney() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery(
                "select sum(money) from tb_outaccount where type ='网购'", null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
    ...
    public double getAllTrafficMoney() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery(
                "select sum(money) from tb_outaccount where type ='出行'", null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
    ...
    public double getAllOtherMoney() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery(
                "select sum(money) from tb_outaccount where type ='其他'", null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
    ...
    public double getOneDayMoney(int day) {
        Calendar c = Calendar.getInstance();
        c.setTime(new Date());
        c.add(c.DAY_OF_YEAR, day);
        String timepattern = new SimpleDateFormat("yyyy-M-d").format(c
                .getTime());
        Log.i("xiayan", timepattern);
        StringBuffer sbsql = new StringBuffer();
        sbsql.append("select sum(money) from tb_outaccount where time ='");
        sbsql.append(timepattern);
        sbsql.append("'");
        Log.i("xiayan", sbsql.toString());
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        // Cursor cursor = db.rawQuery(
        // "select sum(money) from tb_outaccount where time ='2015-5-1'",
        // null);// 获取支出信息的记录数
        Cursor cursor = db.rawQuery(sbsql.toString(), null);// 获取支出信息的记录数
        if (cursor.moveToNext())// 判断Cursor中是否有数据
        {
            return cursor.getDouble(0);// 返回总记录数
        }
